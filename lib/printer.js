...
// ─────────────────────────────────────────────────────────────
//  Moonraker / Klipper — HTTP REST API
// ─────────────────────────────────────────────────────────────

/**
 * Upload a G-code file to a Klipper printer via Moonraker's REST API and
 * optionally start the print immediately.
 *
 * Upload:  POST http://<host>:<port>/server/files/upload
 *           Content-Type: multipart/form-data
 *           Fields: file (the gcode), print ("true" → auto-start after upload)
 * Optional: X-Api-Key header if the instance has trusted-client auth enabled.
 *
 * Moonraker returns { result: { item: { path }, print_started: bool } }.
 */
async function startPrintMoonraker({ host, port = 7125, apiKey, filePath, fileName, start = true, onLog = () => {} }) {
  const url = `http://${host}:${port}/server/files/upload`;
  onLog(`Uploading ${fileName} to Moonraker at ${host}:${port}…`);
  ...
  try {
    ...
    throw new Error(`Could not reach Moonraker at ${host}:${port} — is Klipper running? [${err.message}]`);
  }

  const text = await res.text();
  if (!res.ok) throw new Error(`Moonraker upload rejected: ${res.status} ${text.slice(0, 200)}`);

  let json;
  try { json = JSON.parse(text); } catch { /* non-JSON */ }

  const started = start && json?.result?.print_started === true;
  onLog(start
    ? `Upload complete — print ${started ? 'started!' : 'queued (tap Print on Fluidd/Mainsail/KlipperScreen).'}`
    : 'Upload complete.');
  return json;
}
...
/** Quick reachability check for a known printer (mqtt→:1883, sdcp→:3030, moonraker→:7125). */
function printerOnline(host, protocol, port) {
  const defaultPort = protocol === 'mqtt' ? 1883 : protocol === 'moonraker' ? 7125 : SDCP_PORT;
  return tcpOpen(host, port || defaultPort, 1500);
}

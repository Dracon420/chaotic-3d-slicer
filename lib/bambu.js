...
/**
 * Read the AMS / AMS Lite filament slots from a Bambu printer.
 * Connects to its MQTT-TLS broker and waits for the first status push that
 * includes AMS data, then returns tray info in the same shape as
 * printer.getCanvasFilaments() so the UI can reuse the same filament bar.
 */
function getAmsFilaments({ host, serial, accessCode, timeoutMs = 10000 }) {
  return new Promise((resolve) => {
    const client = mqtt.connect(`mqtts://${host}:8883`, {
      username: 'bblp', password: accessCode,
      rejectUnauthorized: false, reconnectPeriod: 0, connectTimeout: timeoutMs,
    });
    let done = false;
    const finish = (v) => {
    ...
    client.on('connect', () => client.subscribe(`device/${serial}/report`));
    client.on('message', (_topic, payload) => {
      let msg;
      try { msg = JSON.parse(payload.toString()); } catch { return; }
      const amsList = msg?.print?.ams?.ams;
      if (!Array.isArray(amsList) || !amsList.length) return;
      const trays = [];
      for (const unit of amsList) {
        for (const tray of (unit.tray || [])) {
          const colorHex = (tray.tray_color || 'FFFFFF').slice(0, 6);
          trays.push({
            trayId: tray.id != null ? Number(tray.id) : trays.length,
            name: tray.tray_sub_brands || tray.tray_type || 'PLA',
            type: tray.tray_type || 'PLA',
            color: `#${colorHex}`,
            loaded: (tray.remain ?? 100) > 0,
          });
        }
      }
      if (trays.length) finish(trays);
    });
  });
}
...
module.exports = { makeGcode3mf, uploadFile, startPrint, online, getAmsFilaments };

...
/**
 * True if the slicer's desktop GUI (same exe) is already running. Its headless
 * CLI can't slice while the GUI holds the config — that's the #1 cause of a
 * cryptic "CLI::run found error" / crash. Best-effort: any uncertainty (non-
 * Windows, no exe path, tasklist error) resolves false so it never blocks a
 * legitimate slice. Called BEFORE we spawn our own CLI, so any matching process
 * is the GUI (or a stuck previous run), not our slice.
 */
function isSlicerRunning(exePath) {
  return new Promise((resolve) => {
    if (process.platform !== 'win32' || !exePath) return resolve(false);
    const exe = path.basename(exePath);
    exec(`tasklist /FI "IMAGENAME eq ${exe}" /NH`, { windowsHide: true }, (err, stdout) => {
      resolve(!err && typeof stdout === 'string' && stdout.toLowerCase().includes(exe.toLowerCase()));
    });
  });
}
...
function writeReadyPreset(srcPath, type, extra, destDir) {
  ...
  // The CLI validates each loaded config's `from` field and rejects an
  // unrecognised/empty value with "<file>'s from  unsupported" (which is what
  // bit the CC2 presets — their flattened `from` resolved to a value this
  // slicer build doesn't accept). Normalise it to "User", which the CLI always
  // accepts for a user-supplied config. (Confirmed against the real CC2 presets:
  // forcing from="User" makes the previously-failing slice succeed.)
  json.from = 'User';
...
function slice({ exe, presets, overrides = {}, stlPath, outDir, onLog = () => {} }) {
  return new Promise((resolve, reject) => {
    ...
    // cwd: the job folder — when launched from the packaged desktop app the
    // inherited cwd can be a read-only install dir, which some slicer builds
    // dislike; a writable cwd removes that variable.
    const child = spawn(exe, args, { windowsHide: true, cwd: outDir });

    let lastError = '';
    const wire = (stream) => (chunk) => {
      for (const line of chunk.toString().split(/\r?\n/)) {
        if (!line.trim()) continue;
        if (/\[error\]|error|not compatible|unsupported|exit|invalid|cannot|failed/i.test(line)) lastError = line;
        onLog(stream, line);
      }
    };
    ...
    child.on('close', (code) => {
      ...
      if (code === 0 && gcode) {
        ...
        // 0xC0000005 — the slicer itself crashed (access violation).
        reject(new Error(
          'The slicer crashed while starting up. Two common fixes: ' +
          '(1) CLOSE the ElegooSlicer window — its CLI can crash while the GUI is open; ' +
          '(2) update ElegooSlicer to the latest version (tested with 1.5.1.6). Then slice again.'
        ));
      } else if (/unsupported|not compatible|invalid/i.test(lastError)) {
        // The slicer rejected a preset (often a CC2 machine preset on an older
        // ElegooSlicer build, or a preset re-saved by a newer GUI). Surface the
        // exact line and the fix.
        reject(new Error(
          `${lastError}\n→ This is the slicer rejecting a preset. On the slicing PC: update ElegooSlicer to the latest version, then open it once and re-save your printer/process/filament presets (so they're written in this version's format). Close the ElegooSlicer window, then slice again.`
        ));
      } else {
...
module.exports = { slice, prepareCliPresets, isSlicerRunning };
